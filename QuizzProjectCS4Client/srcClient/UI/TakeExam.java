...
public class TakeExam extends javax.swing.JPanel {
    ...
    public TakeExam(int width, ExamDTO exam, UserDTO user) throws Exception {
        ...
        this.exam = exam;
        this.user = user;
        myresult = new MyExamResult(exam.getId(), 0, 0, 0, 0, 1);
        initComponents();
        this.setSize(this.DEFALUT_WIDTH - 200, 750);
        initComponents();
        init();
...
//              JOptionPane.showMessageDialog(null,"Kết Quả","Điểm :"+point);
    ...
    private void btnResultActionPerformed(java.awt.event.ActionEvent evt) throws IOException {//GEN-FIRST:event_btnResultActionPerformed
        Notify notipn = new Notify(ManageExam.getFrames()[0], false);
        notipn.setInfo("An", myresult.getAmount_correct(), myresult.getAmount_incorrect(), (exam.getTotal_time() * 60000) - TimeExam,
